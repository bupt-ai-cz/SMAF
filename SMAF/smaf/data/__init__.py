...
def find_dataset_using_name(name):                   #返回dataset 下一步用于dataloader
...
#########################################################
#my dataset
#def create_voc_coo_dataset(opt, logger):
...
#     return Compose([VOC_Scaling([0.5, 0.75, 1.0, 1.25, 1.5]),
#                     RandomCrop(opt.rcrop),
#                     RandomHorizontallyFlip(opt.hflip)])
...
#         target_train = find_dataset_using_name(opt.tgt_dataset)
        self.target_train = VOCAug_stage1(opt, logger, augmentations=data_aug, split='train_cls')
...
#         target_valid = find_dataset_using_name(opt.tgt_dataset)
        self.target_valid = VOCAug_stage1(opt, logger, augmentations=None, split='val')
