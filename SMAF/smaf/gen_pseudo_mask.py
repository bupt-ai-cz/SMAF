...
def _crf_with_alpha(image, cam_dict, alpha, t=10):
    ...
    crf_score = _crf_inference(image, predict)
...
#                 tensor_crf = np.zeros((opt.num_classes + 1, h, w), np.float32)
#                 for key in cam_crf.keys():
#                     tensor_crf[key] = cam_crf[key]
                
#                 print("debug tensor_crf.shape {}".format(tensor_crf.shape))
#                 np.save(os.path.join(folder, img_id + '.npy'), cam_crf)
#                 pred_crf = np.argmax(tensor_crf, axis=0).astype(np.uint8)
#                 print("debug pred_crf .shape {}".format(pred_crf.shape))
                imageio.imwrite(os.path.join(opt.crf_png, img_id + '.png'), cam_crf)
...
#     args = parse_args()
    ...
    if opt.crf_png is not None:
        os.makedirs(opt.crf_png, exist_ok=True)
        opt.save_type.append(opt.crf_png)
    ...
    if opt.crf:
        ...
        for t in opt.crf_t:
            for alpha in opt.crf_alpha:
                ...
                opt.save_type.append(crf_folder)
