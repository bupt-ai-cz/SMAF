...
#         self.logger = logger
        ...
        self.nets_DP = []
        self.default_gpu = 0
        self.objective_vectors = torch.zeros([self.class_numbers, 4096])
        ...
        if self.opt.student_init == 'imagenet':
            self.BaseNet = Deeplab(BatchNorm, num_classes=self.class_numbers, freeze_bn=True,restore_from=restore_from)
...
#         logger.info('the backbone is {}'.format(opt.model_name))
        ...
        if self.opt.finetune or self.opt.stage == 'warm_up':
            ...
            self.BaseOpti = optimizer_cls(params = [{"params":self.get_params(self.BaseNet_DP.module,key="1x"), "lr":optimizer_params['lr']},{'params':self.get_params(model = self.BaseNet_DP.module, key="10x"), 'lr':optimizer_params['lr']*10}], **optimizer_params)
                
        else:
            self.BaseOpti = self.optimizer
#             self.BaseOpti = optimizer_cls(self.BaseNet_DP.parameters(), **optimizer_params)
...
#             print("source_x shape {}".format(source_x.shape))
            source_output = self.BaseNet_DP(source_x)
#             print("source_output['out'] shape {}".format(source_output['out'].shape))
            source_outputUp = F.interpolate(source_output['out'], size=source_x.size()[2:], mode='bilinear', align_corners=True)
    ...
    def step(self,img_id, img, saliency, label):
        with autocast():
...
#         return loss_cls.item(),loss_sal.item(),loss.item()
        ...
        if self.opt.proto_rectify:
            ...
            ###save EPS_PM
            if(epoch<50):
                Eps_pm_path = "eps_pm_at_{}".format(epoch)
                if not os.path.exists(Eps_pm_path):
                    os.makedirs(Eps_pm_path)
                mask1_temp = pseudo_mask_warmup.clone().detach().cpu()#.numpy()

                for counter in range(mask1_temp.shape[0]):
                    img_id_temp = img_id[counter]
                    img_temp_i = mask1_temp[counter,:,:]
                    img_temp_i = np.squeeze(img_temp_i)
        #                 img_temp_i = img_temp_i.transpose(1,2,0)
                    img_temp_i = Image.fromarray(np.uint8(img_temp_i))
                    temp_save_pth = os.path.join(Eps_pm_path,str(img_id_temp)+".png")
                    img_temp_i.save(temp_save_pth)
            ###
            if(epoch<50):
                gt_path = "gt_at_{}".format(epoch)
                if not os.path.exists(gt_path):
                    os.makedirs(gt_path)
                mask1_temp = img_gt.clone().detach().cpu()#.numpy()
        ...
        #                 img_temp_i = img_temp_i.transpose(1,2,0)
                    img_temp_i = Image.fromarray(np.uint8(img_temp_i))
                    temp_save_pth = os.path.join(gt_path,str(img_id_temp)+".png")
                    img_temp_i.save(temp_save_pth)
            
            
            
            mask1 = threshold_arg.max(1, keepdim=True)[1].bool().int()
            
            if(epoch==0):
                mask1_temp = mask1.clone().detach().cpu().numpy()
                ...
                for counter in range(mask1_temp.shape[0]):
                    ...
                    img_temp_i = np.squeeze(img_temp_i)
    #                 img_temp_i = img_temp_i.transpose(1,2,0)
                    img_temp_i = Image.fromarray(np.uint8(img_temp_i*255))
                    temp_save_pth = os.path.join("debug_mask1",str(img_id_temp)+"_mask.png")
                    img_temp_i.save(temp_save_pth)
...
#             weights = self.get_prototype_weight(target_out['feat'])
            rectified = weights * threshold_arg
#             print("weights shape {},threshold_arg shape {},rectified shape {},ema_out['feat'] shape {}".format(weights.shape,threshold_arg.shape,rectified.shape,ema_out['feat'].shape))
            #####################################################################################
            ###
            if(epoch<50):
                lp_soft_pm_path = "lp_soft_aug_at_{}".format(epoch)
                if not os.path.exists(lp_soft_pm_path):
                    os.makedirs(lp_soft_pm_path)
                mask1_temp = threshold_arg.clone().detach().cpu()#.numpy()
                mask1_temp = mask1_temp.max(1, keepdim=True)[1].numpy()
                for counter in range(mask1_temp.shape[0]):
                    img_id_temp = img_id[counter]
                    img_temp_i = mask1_temp[counter,:,:,:]
                    img_temp_i = np.squeeze(img_temp_i)
        #                 img_temp_i = img_temp_i.transpose(1,2,0)
            #                         print("img_temp shape {}".format(img_temp_i.shape))
                    img_temp_i = Image.fromarray(np.uint8(img_temp_i))
                    temp_save_pth = os.path.join(lp_soft_pm_path,str(img_id_temp)+".png")
                    img_temp_i.save(temp_save_pth)
    
            ###########################
            rectified[:,0,:,:] = 0
            B,C,H,W = rectified.shape
            img_cls_label = img_cls_label.reshape(B,C,1,1).expand(B,C,H,W)

            rectified = rectified*img_cls_label

    
            ##############################
            threshold_arg = rectified.max(1, keepdim=True)[1]
            ###
            threshold_arg = threshold_arg*mask1
            
            if(epoch==0):
                threshold_arg_temp = threshold_arg.clone().detach().cpu().numpy()
                for counter in range(threshold_arg_temp.shape[0]):
                    img_id_temp = img_id[counter]
                    img_temp_i = threshold_arg_temp[counter,:,:,:]
                    img_temp_i = np.squeeze(img_temp_i)
    #                 img_temp_i = img_temp_i.transpose(1,2,0)
                    img_temp_i = Image.fromarray(np.uint8(img_temp_i))
                    temp_save_pth = os.path.join("debug_threshold_arg",str(img_id_temp)+".png")
                    img_temp_i.save(temp_save_pth)
            ###
            rectified = rectified / rectified.sum(1, keepdim=True)
            argmax = rectified.max(1, keepdim=True)[0]
            threshold_arg[argmax < self.opt.train_thred] = 255
            
            #########################################################################

            if(epoch<50):
                rectified_pm_path = "pm_rectified_at_{}".format(epoch)
                if not os.path.exists(rectified_pm_path):
                    os.makedirs(rectified_pm_path)
                mask1_temp = threshold_arg.clone().detach().cpu().numpy()
                for counter in range(mask1_temp.shape[0]):
                    img_id_temp = img_id[counter]
                    img_temp_i = mask1_temp[counter,:,:,:]
        ...
        #                 img_temp_i = img_temp_i.transpose(1,2,0)
            #                         print("img_temp shape {}".format(img_temp_i.shape))
                    img_temp_i = Image.fromarray(np.uint8(img_temp_i))
                    temp_save_pth = os.path.join(rectified_pm_path,str(img_id_temp)+".png")
                    img_temp_i.save(temp_save_pth)
            ##############################################################################
        ...
        if self.opt.S_pseudo > 0:
            threshold_argS = self.label_strong_T(threshold_arg.clone().float(), target_params, padding=255, scale=4).to(torch.int64)
            cluster_argS = self.label_strong_T(cluster_arg.clone().float(), target_params, padding=255, scale=4).to(torch.int64)
        ...
        if self.opt.proto_consistW > 0:
            ...
            ema2strong_feat_proto_distance = self.label_strong_T(ema2weak_feat_proto_distance, target_params, padding=255, scale=4)
            mask = (ema2strong_feat_proto_distance != 255).float()
    ...
    def rce(self, pred, labels):
        ...
        labels[labels==255] = self.class_numbers
    ...
    def step_distillation(self, source_x, source_label, target_x, target_imageS=None, target_params=None, target_lp=None):
        ...
        threshold_arg = F.interpolate(target_lp.unsqueeze(1).float(), scale_factor=0.25).long()
        if self.opt.S_pseudo > 0:
            threshold_arg = self.label_strong_T(threshold_arg.clone().float(), target_params, padding=255, scale=4).to(torch.int64)
...
#             net = DataParallelWithCallback(net, device_ids=range(torch.cuda.device_count()))
    ...
    def eval(self, net=None, logger=None):
        ...
        if net == None:
            for net in self.nets:
                net.eval()
#                 print("***********************successfully nets.eval")
            for net in self.nets_DP:
                net.eval()
#                 print("*********************** successfully nets_DP.eval")
#             if logger!=None:    
...
#                 logger("Successfully set {} eval mode".format(net.__class__.__name__))
    ...
    def train(self, net=None, logger=None):
        if net==None:
            for net in self.nets:
                net.train()
#                 print("***********************successfully nets.train")
            for net in self.nets_DP:
                net.train()
#                 print("*********************** successfully nets_DP.train")
        else:
            net.train()
