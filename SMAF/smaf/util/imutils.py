...
def random_crop_with_saliency(imgarr, mask,gth, crop_size):
    param = {}
    h, w, c = imgarr.shape
    ...
    container_gth = np.zeros((crop_size, crop_size, imgarr.shape[-1]), np.float32)
    
    container[cont_top:cont_top+ch, cont_left:cont_left+cw] = \
        imgarr[img_top:img_top+ch, img_left:img_left+cw]
    container_mask[cont_top:cont_top+ch, cont_left:cont_left+cw] = \
        mask[img_top:img_top+ch, img_left:img_left+cw]
    container_gth[cont_top:cont_top+ch, cont_left:cont_left+cw] = \
        gth[img_top:img_top+ch, img_left:img_left+cw]
    
    param["cont_top"] = cont_top
    param["ch"] = ch
    param["cont_left"] = cont_left
    param["cw"] = cw
    param["img_top"] = img_top
    param["img_left"] = img_left
#     print("origion param crop {}".format(param))
    

    return container, container_mask, param,container_gth
